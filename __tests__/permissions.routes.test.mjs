...
await jest.isolateModulesAsync(async () => {
    ({ app } = await import("../src/app.js")); // tu Express app que monta /permissions
...
describe("Permissions Exceptions API (HU-005)", () => {
    test("POST /permissions/exceptions -> 201", async () => {
        accessExceptionService.apply.mockResolvedValue({ userId: 1, documentId: 2, permissions: ["VIEW"] });
    ...
    test("GET /permissions/exceptions -> 200", async () => {
        accessExceptionService.list.mockResolvedValue([{ userId: 1, documentId: 2, permissions: "EDIT,VIEW" }]);
    ...
    test("POST valida 400 cuando service lanza code=400", async () => {
        accessExceptionService.apply.mockRejectedValue(Object.assign(new Error("reason is required"), { code: 400 }));
        const res = await request(app).post("/permissions/exceptions").send({ userId: 1, documentId: 2 });
