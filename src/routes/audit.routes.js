...
import {
  ...
  listAllDocumentStates,
  getAuditEventDetailById,
...
router.get("/events", async (req, res) => {
  try {
    ...
    const sortCol = ALLOWED_SORT.has(String(sortBy))
      ? String(sortBy)
      : "fecha_hora";
...
router.get("/events/csv", async (req, res) => {
  try {
    ...
    if (!result.items || result.items.length === 0) {
      return res
        .status(404)
        .json({ message: "No hay datos disponibles para exportar." }); // No data found
    }
...
router.get("/documents/states", async (_req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to retrieve document states" });
  }
});
