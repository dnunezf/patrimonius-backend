...
export const documentoRepo = {
  ...
  async updateContenido(id, contenido) {
    await pool.query(`UPDATE Documento SET contenido = ? WHERE id = ?`, [
      contenido,
      id,
    ]);
    return this.findById(id);
  ...
  async updateEstado(id, estado) {
    await pool.query(`UPDATE Documento SET estado = ? WHERE id = ?`, [
      estado,
      id,
    ]);
    return this.findById(id);
  ...
  async updateNumeroSerie(id, numero_serie) {
    await pool.query(`UPDATE Documento SET numero_serie = ? WHERE id = ?`, [
      numero_serie,
      id,
    ]);
    return this.findById(id);
  ...
  async sign(documentId, userId = null) {
    if (!userId) {
      const [r] = await pool.query(
        `SELECT usuario_id FROM Documento WHERE id = ?`,
        [documentId],
      );
