...
export const conservationIntakeRepo = {
  ...
  async listDocumentSignatures(documentId) {
    const [rows] = await pool.query(
      `
        ...
        FROM Firma_Digital fd
        JOIN Usuario u ON u.id = fd.usuario_id
  ...
  async findEadDocumentById(documentId) {
    const [rows] = await pool.query(
      `
        SELECT
          d.id,
          ...
          ic.id AS intakeId,
          ic.official_code AS intakeOfficialCode,
          ic.classification_code AS classificationCode,
          ic.classification_label AS classificationLabel,
          ic.retention_years AS retentionYears,
          ic.retention_start_date AS retentionStartDate,
          ic.retention_end_date AS retentionEndDate,
