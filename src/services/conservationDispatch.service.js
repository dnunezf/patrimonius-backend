...
function canActorDispatchDocument(actor, documentRow) {
    if (!actor?.id) return false;
    if (actor?.isMaster) return true;

    const rolIds = Array.isArray(actor?.rolIds)
        ? actor.rolIds.map((item) => Number(item))
        : [];

    // 1: Administrador. 2/3 se dejan habilitados para editor/archivista si en tu sistema usan esos ids.
    if (rolIds.includes(1) || rolIds.includes(2) || rolIds.includes(3)) {
        return true;
    }
    ...
    if (
        ...
        Number(documentRow?.unitId || 0) === Number(actor.unidadId || 0)
    ) {
        return true;
    }
...
export const conservationDispatchService = {
    async getDispatchDetail(documentId, actor) {
        ...
        const doc = await conservationDispatchRepo.findConservationDocumentById(
            documentId,
        );
        ...
        if (!canActorDispatchDocument(actor, doc)) {
            throw buildError(
    ...
    async sendDispatchEmail(documentId, rawPayload, actor) {
        ...
        for (const attachmentId of optionalAttachmentIds) {
            if (typeof conservationDispatchRepo.findDispatchAnexoById !== "function") {
                continue;
            }
...
Código: ${doc.officialCode || "—"}`,
            ...
            const expId = Number(doc?.payloadSnapshot?.classification?.expedienteId || 0);
