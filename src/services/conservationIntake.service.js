...
/**
 * Siglas oficiales de unidades organizacionales.
 * Se agregan aquí para NO tocar la base de datos.
 *
 * Formato final esperado:
 * TIPO-MNCR-SIGLAS-###-AÑO
 *
 * Ejemplos:
 * SOL-MNCR-JA-001-2026
 * OFI-MNCR-DG-AJ-001-2026
 * INF-MNCR-DAF-AFC-001-2026
 * SOL-MNCR-DPPC-001-2026
 */
const UNIT_CODE_MAP = {
    // Sub Fondo / Unidades principales
    "JUNTA ADMINISTRATIVA": "JA",
    "JA": "JA",
    ...
    "ASESORIA JURIDICA": "DG-AJ",
    "ASESORIA LEGAL": "DG-AJ",
    "DG AJ": "DG-AJ",
    "DG-AJ": "DG-AJ",

    "UNIDAD DE INFORMATICA": "DG-UI",
    "INFORMATICA": "DG-UI",
    "DG UI": "DG-UI",
    "DG-UI": "DG-UI",

    "PROGRAMA DE MUSEOS REGIONALES Y COMUNITARIOS": "DG-PMRC",
    "PROGRAMA MUSEOS REGIONALES Y COMUNITARIOS": "DG-PMRC",
    "MUSEOS REGIONALES Y COMUNITARIOS": "DG-PMRC",
    "PMRC": "DG-PMRC",
    "DG PMRC": "DG-PMRC",
    "DG-PMRC": "DG-PMRC",

    "SITIO MUSEO FINCA 6": "DG-CVSMF6",
    "CENTRO DE VISITANTES SITIO MUSEO FINCA 6": "DG-CVSMF6",
    "CENTRO VISITANTES SITIO MUSEO FINCA 6": "DG-CVSMF6",
    "CVSMF6": "DG-CVSMF6",
    "DG CVSMF6": "DG-CVSMF6",
    "DG-CVSMF6": "DG-CVSMF6",

    // Departamento de Administracion y Finanzas
    "DEPARTAMENTO DE ADMINISTRACION Y FINANZAS": "DAF",
    "ADMINISTRACION Y FINANZAS": "DAF",
    "DAF": "DAF",

    "FINANCIERO CONTABLE": "DAF-AFC",
    "AREA FINANCIERO CONTABLE": "DAF-AFC",
    "AREA DE FINANCIERO CONTABLE": "DAF-AFC",
    "AFC": "DAF-AFC",
    "DAF AFC": "DAF-AFC",
    "DAF-AFC": "DAF-AFC",

    "OFICINA AUXILIAR DE GESTION INSTITUCIONAL DE RECURSOS HUMANOS": "DAF-GIRHA",
    "GESTION INSTITUCIONAL DE RECURSOS HUMANOS": "DAF-GIRHA",
    "GESTION DE RECURSOS HUMANOS": "DAF-GIRHA",
    "RECURSOS HUMANOS": "DAF-GIRHA",
    "GIRHA": "DAF-GIRHA",
    "DAF GIRHA": "DAF-GIRHA",
    "DAF-GIRHA": "DAF-GIRHA",

    "SERVICIOS GENERALES": "DAF-SG",
    "SG": "DAF-SG",
    "DAF SG": "DAF-SG",
    "DAF-SG": "DAF-SG",

    "PROVEEDURIA INSTITUCIONAL": "DAF-PI",
    "PI": "DAF-PI",
    "DAF PI": "DAF-PI",
    "DAF-PI": "DAF-PI",

    "ARCHIVO CENTRAL": "DAF-AC",
    "AC": "DAF-AC",
    "DAF AC": "DAF-AC",
    "DAF-AC": "DAF-AC",

    "ARQUITECTO": "DAF-ARQ",
    "ARQUITECTURA": "DAF-ARQ",
    "ARQ": "DAF-ARQ",
    "DAF ARQ": "DAF-ARQ",
    "DAF-ARQ": "DAF-ARQ",

    // Otros departamentos principales
    "DEPARTAMENTO DE PROYECCION MUSEOLOGICA": "DPM",
    "PROYECCION MUSEOLOGICA": "DPM",
    "DPM": "DPM",

    "DEPARTAMENTO DE PROTECCION DEL PATRIMONIO CULTURAL": "DPPC",
    "PROTECCION DEL PATRIMONIO CULTURAL": "DPPC",
    "PROTECCION PATRIMONIO CULTURAL": "DPPC",
    "DPPC": "DPPC",

    // IMPORTANTE:
    // Si el frontend o la BD mandan PPC, se interpreta como DPPC,
    // porque en la tabla oficial aparece como:
    // Departamento de Proteccion del Patrimonio Cultural -> DPPC.
    "PPC": "DPPC",

    "DEPARTAMENTO DE ANTROPOLOGIA E HISTORIA": "DAH",
    "ANTROPOLOGIA E HISTORIA": "DAH",
    "DAH": "DAH",

    "DEPARTAMENTO DE HISTORIA NATURAL": "DHN",
    "HISTORIA NATURAL": "DHN",
    "DHN": "DHN",
};
...
function resolveOfficialUnitSuffix(producingUnit) {
    const normalized = normalizeComparableLabel(producingUnit);
    if (!normalized) return null;

    // 1. Coincidencia exacta normalizada.
    if (UNIT_CODE_MAP[normalized]) {
        return UNIT_CODE_MAP[normalized];
    }

    // 2. Si viene con codigo numerico al inicio, por ejemplo:
    // "006 Departamento de Proteccion del Patrimonio Cultural"
    // "004.5 Archivo Central"
    const withoutLeadingNumber = normalized
        .replace(/^\d+(?:\.\d+)*\s+/, "")
        .trim();

    if (withoutLeadingNumber && UNIT_CODE_MAP[withoutLeadingNumber]) {
        return UNIT_CODE_MAP[withoutLeadingNumber];
    }

    // 3. Si viene con MNCR adelante, por ejemplo:
    // "MNCR DPPC" o "MNCR-DPPC" normalizado como "MNCR DPPC".
    const withoutInstitution = normalized
        .replace(/^MNCR\s+/, "")
        .trim();

    if (withoutInstitution && UNIT_CODE_MAP[withoutInstitution]) {
        return UNIT_CODE_MAP[withoutInstitution];
    }

    // 4. Si viene con prefijo institucional y codigo numerico:
    // "MNCR 006 Departamento de Proteccion del Patrimonio Cultural"
    const withoutInstitutionAndNumber = withoutInstitution
        .replace(/^\d+(?:\.\d+)*\s+/, "")
        .trim();

    if (
        withoutInstitutionAndNumber &&
        UNIT_CODE_MAP[withoutInstitutionAndNumber]
    ) {
        return UNIT_CODE_MAP[withoutInstitutionAndNumber];
    }
...
function buildUnitSuffix(producingUnit) {
    const officialSuffix = resolveOfficialUnitSuffix(producingUnit);
    if (officialSuffix) return officialSuffix;
...
export const conservationIntakeService = {
    ...
    async buildReferenceCodeForExistingDocument(documento_id, overrides = {}) {
        ...
        const resolvedProducingUnit =
            ...
            String(
                pickFirst(metadataMap, [
                    FINAL_KEYS.PRODUCING_UNIT,
                    "ARCH_PRODUCING_UNIT",
                    "EDIT_AUTO_PRODUCER_UNIT_NAME",
                    "DESC_RESPONSIBLE_UNIT_NAME",
                    "UNIDAD_PRODUCTORA",
                    "UNIDAD_PRODUCTORA_NOMBRE",
                ]) || "",
            ).trim() ||
            String(doc.producingUnitName || "").trim() ||
            String(doc.unidad_nombre || "").trim() ||
            String(doc.unitName || "").trim();

        if (!resolvedProducingUnit) {
            const error = new Error(
                "Producing unit is required to generate the final reference code",
            );
            error.code = "INCOMPLETE_ARCHIVAL_METADATA";
            throw error;
        }
    ...
    async registerIntake(rawPayload, actor) {
        ...
        const resolvedProducingUnit = String(
            payload.metadata.producingUnit ||
            pickFirst(docMetadataMap, [
                FINAL_KEYS.PRODUCING_UNIT,
                "ARCH_PRODUCING_UNIT",
                "EDIT_AUTO_PRODUCER_UNIT_NAME",
